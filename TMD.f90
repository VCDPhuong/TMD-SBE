...
module globalvar
    ...
    module para_MX2
     ...
     subroutine gradient_H(kx, ky, grad_H)
        ...
        implicit none
        double precision, intent(In) :: kx, ky
        complex*16, allocatable, dimension(:,:) :: hamu_plus, hamu_minus, hamd_plus, hamd_minus
        complex*32, dimension(6,6,2), INTENT(OUT) :: grad_H
        real :: delta = 5e-2
        allocate(hamd_plus(3,3), hamd_minus(3,3), hamu_plus(3,3), hamu_minus(3,3))
        if (kx /=0) then
           call tbham(kx*(1 + delta), ky, hamu_plus , hamd_plus )
           call tbham(kx*(1 - delta), ky, hamu_minus, hamd_minus)
           grad_H(1:3,1:3,1) = (hamu_plus - hamu_minus) / (2 * delta * kx)
           grad_H(4:6,4:6,1) = (hamd_plus - hamd_minus) / (2 * delta * kx)
        else
        ...
        if (ky /=0) then
           call tbham(kx, ky*(1 + delta), hamu_plus , hamd_plus )
           call tbham(kx, ky*(1 - delta), hamu_minus, hamd_minus)
           grad_H(1:3,1:3,2) = (hamu_plus - hamu_minus) / (2 * delta * ky)
           grad_H(4:6,4:6,2) = (hamd_plus - hamd_minus) / (2 * delta * ky)
        else
     ...
     module varia
        ...
        complex*16 ,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax) :: V
        complex*16 ,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1) :: p,Xi
        ...
        use globalvar
        implicit none
        double precision,dimension(0:Norbital,0:nkamax,0:nkamax),intent(out) :: Es
        double precision,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax),intent(out) :: deltaEs
        complex*16 ,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1),intent(out) :: ps, xis
        double precision,dimension(0:nkamax,0:nkamax,0:1),intent(out) :: grids
        integer,dimension(0:Norbital,0:Norbital),intent(out) :: Deltas
        double precision,dimension(0:1,0:1) :: B
        complex*16,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax),intent(out) :: Vs
        complex*32, dimension(0:Norbital,0:Norbital) :: Vpkx, Vpky
        complex*32, dimension(0:Norbital,0:Norbital,0:1) :: Vpkxy
        complex*16, dimension(0:halfNorbital,0:halfNorbital) :: hamu,hamd
        ...
        do nk2 = 0,nkamax
            do nk1 =0,nkamax
                ...
                grids(nk1,nk2,0) = grid(nk1,nk2,0) - 2.*pi/a
                grids(nk1,nk2,1) = grid(nk1,nk2,1) - 0.
            end do
        end do
            open(unit=1,FILE='OUTPUT/grid.txt')
            do nk2 = 0,nkamax
                do nk1 =0,nkamax
                write(1,'(E12.5, 2x, E12.5)') grids(nk1,nk2,0), grids(nk1,nk2,1)
                end do
            end do
            close(1)
            ...
            do nk2 = 0,nkamax
               do nk1 =0,nkamax
                    ...
                    Es(0:2,nk1,nk2) = E_sorted
                    Vs(0:2,0:2,nk1,nk2)=hamu
                    call zheev('V','U',3,hamd,3,E_sorted,WORK,5,RWORK,INFO)
                    !final index is 0 mean eigenvalue is ascending order
                    !sort eigenvalue and eigenvector
                    Es(3:5,nk1,nk2) = E_sorted
                    Vs(3:5,3:5,nk1,nk2)=hamd
                    write(1,'(F12.6, 2x, F12.6, 2x, F12.6, 2x, F12.6, 2x, F12.6, 2x, F12.6, 2x, F12.6, 2x, F12.6)')&
                    kx, ky, Es(0,nk1,nk2),Es(1,nk1,nk2),Es(2,nk1,nk2),Es(3,nk1,nk2),Es(4,nk1,nk2),Es(5,nk1,nk2)
                    call gradient_H(kx ,ky ,Vpkxy)
                    Vpkx=Vpkxy(:,:,0)
                    Vpky=Vpkxy(:,:,1)
                    do mu = 0, Norbital
                        do nu = 0, Norbital
                        ps(nu,mu,nk1,nk2,0)=me/hbar*DOT_PRODUCT(Vs(:,nu,nk1,nk2),matmul(Vpkx, Vs(:, mu, nk1, nk2)))
                        ps(nu,mu,nk1,nk2,1)=me/hbar*DOT_PRODUCT(Vs(:,nu,nk1,nk2),matmul(Vpky, Vs(:, mu, nk1, nk2)))
                        if (nu>=mu) then
                        write(2,'(I5, 2x, I5, 2x, F12.6, 2x, F12.6, 2x, F15.10, 2x, F15.10, 2x, F15.10, 2x, F15.10)')&
                            nu ,mu ,kx ,ky ,real(ps(nu,mu,nk1,nk2,0)) ,imag(ps(nu,mu,nk1,nk2,0)),&
                            real(ps(nu,mu,nk1,nk2,1)) ,imag(ps(nu,mu,nk1,nk2,1))
                        end if
                        deltaEs(nu,mu,nk1,nk2)=Es(nu,nk1,nk2)-Es(mu,nk1,nk2)
                            if (abs(deltaEs(nu, mu, nk1, nk2)) < 1e-2) then
                        ...
                        do mu = 0, Norbital
                            ...
                            if (nu > mu) then
                                write(1, '(I5, 2x, I5, 2x, F12.6, 2x, F12.6, 2x, F15.10, 2x, F15.10, 2x, F15.10, 2x, F15.10)') &
                                    nu, mu, grids(nk1, nk2, 0), grids(nk1, nk2, 1), real(xis(nu, mu, nk1, nk2, 0)), &
                                    imag(xis(nu, mu, nk1, nk2, 0)), real(xis(nu, mu, nk1, nk2, 1)), imag(xis(nu, mu, nk1, nk2, 1))
                            end if
                        end do
                    end do
                end do
    ...
    !end subroutine
        ...
        use globalvar
        implicit none
        double complex, dimension(:,:,:,:), intent(inout) :: rhos
        double complex, dimension(:,:,:,:,:), intent(in) :: ps
        double precision, dimension(:,:,:,:), intent(in) :: deltaEs
        double complex,allocatable, dimension(:,:,:,:,:) :: Rdummy
        integer, intent(in) :: nts
        double precision, dimension(:,:), intent(in) :: Ats
        real :: time
        allocate(Rdummy(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:4))
        ! Calculate k1
        time = tmin + deltat * nts
        Rdummy(:,:,:,:,0) = rhos
        call RHSsbeVG(Rdummy(:,:,:,:,1), rhos, time, ps, deltaEs, Ats)
    
        ! Calculate k2
        call RHSsbeVG(Rdummy(:,:,:,:,2), rhos + deltat * 0.5* Rdummy(:,:,:,:,1), time+ 0.5*deltat,  ps, deltaEs, Ats)
    
        ! Calculate k3
        call RHSsbeVG(Rdummy(:,:,:,:,3), rhos + deltat * 0.5* Rdummy(:,:,:,:,2), time+ 0.5*deltat,  ps, deltaEs, Ats)
    
        ! Calculate k4
        call RHSsbeVG(Rdummy(:,:,:,:,4), rhos + deltat * Rdummy(:,:,:,:,3), time+ deltat,  ps, deltaEs, Ats)
    ...
    module calva
        ...
        subroutine calculatev( rhos, nts, xis, ps, Ats)
            ...
            call tb_parameters(comid)
            deltak=4.*pi/(sqrt(3.)*a*nkamax)
            allocate(N(0:Norbital) ,P(0:1) ,I(0:1) , velo(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1))
            P = complex(1,0)
                ...
                do j=0,1
                    do nk2=0, nkamax
                        do nk1=0, nkamax
                            do mu = 0 ,Norbital
                                do nu = 0, Norbital
                                    velo(nu,mu,nk1,nk2,j)=(ps(nu,mu,nk1,nk2,j)-qe*At(j))/me
                                    if (nu/=mu) then
                                    P(j)=P(j)+1/(2*pi)**2*(xis(nu,mu,nk1,nk2,j)*rhos(mu,nu,nk1,nk2))*dS
                                    end if
                                    I(j)=I(j)+1/(2*pi)**2*(velo(nu,mu,nk1,nk2,j)*rhos(mu,nu,nk1,nk2))*dS
                                end do
                            end do
                        end do
                    end do
    ...
    program main
        ...
        !        print *, "-------------------------------------------------------------------------------------------------"
        !        write(*,'(A,F8.2,A,F8.2,A,F4.2)')"Real time:", real(tmin+nt*deltat)," on ",tmax, ". Time per step ", finish-start 
        !        write(*,'(F8.2,x,F8.2,x,F8.2,x,F8.2,x,F8.2,x,F8.2)') &
