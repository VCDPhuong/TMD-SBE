...
module globalvar
     ...
     module varia
        ...
        do nk2 = 0,nkamax
            do nk1 =0,nkamax
                ...
                grids(nk1,nk2,0)=grids(nk1,nk2,0)-2*pi/a
                grids(nk1,nk2,1)=grids(nk1,nk2,1)-0
            end do
        end do
            open(unit=1,FILE='grid.txt')
            do nk2 = 0,nkamax
                do nk1 =0,nkamax
                write(1,'(E12.5, 2x, E12.5, 2x, E12.5, 2x, E12.5)') real(nk1), real(nk2), grids(nk1,nk2,0), grids(nk1,nk2,1)
                end do
            end do
            close(1)
            print *, "grid done"
    ...
    subroutine El(Elecs,Alecs)
        use globalvar
        implicit none
        double precision, dimension(0:ntmax,0:1) :: Alecs,Elecs
    ...
    module def
        ...
        subroutine RHSsbeVG(RHSs, rhos , nt1s ,ps ,deltaEs, Ats)
            ...
            double complex,   dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax), intent(in) :: ps
            double precision, dimension(:,:), intent(in) :: Ats
            ...
            RHSs=-cmplx(0,1)/hbar*deltaEparts - cmplx(0,1)/hbar*qe*lightinteractionparts
            deallocate(lightinteractionparts,dummy1s,deltaEparts)
        end subroutine
        subroutine RK4(rhos, nts, ps, deltaEs)
        use globalvar
        implicit none
        ...
        Rdummy(:,:,:,:,0) = rhos
        call RHSsbeLG(Rdummy(:,:,:,:,1), rhos, time, ps, deltaEs)
    
        ! Calculate k2
        call RHSsbeLG(Rdummy(:,:,:,:,2), rhos + deltat * 0.5* Rdummy(:,:,:,:,1), time+ 0.5*deltat,  ps, deltaEs)
    
        ! Calculate k3
        call RHSsbeLG(Rdummy(:,:,:,:,3), rhos + deltat * 0.5* Rdummy(:,:,:,:,2), time+ 0.5*deltat,  ps, deltaEs)
    
        ! Calculate k4
        call RHSsbeLG(Rdummy(:,:,:,:,4), rhos + deltat * Rdummy(:,:,:,:,3), time+ deltat,  ps, deltaEs)
    ...
    !double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax), intent(out) :: gradrho
    !integer :: i
    !do i=0,nkamax
    !if (i==0) then
