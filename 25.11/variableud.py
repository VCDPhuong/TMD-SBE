import numpy as np
from hamud import Hamu,Hamd,H,Vpkx,Vpky,a,Norbital
...
def variable():
    ...
    for nk1 in range(nkamax+1):
        for nk2 in range(nkamax+1):
            k=grid[nk1,nk2]
            E[0:3,nk1,nk2],V[0:3,0:3,nk1,nk2]=LA.eigh(Hamu(k[0],k[1]))
            E[3:6,nk1,nk2],V[3:6,3:6,nk1,nk2]=LA.eigh(Hamd(k[0],k[1]))
        ...
        for nk2 in range(nkamax+1):
            ...
            for nu in range(0,Norbital):
                for mu in range(0,Norbital):
                    p[nu,mu,nk1,nk2,0]=me/hbar*(np.transpose(np.conj(V[:,nu,nk1,nk2]))@Vpkx(k[0],k[1])@V[:,mu,nk1,nk2]) #[fs/nm*eV]
                    p[nu,mu,nk1,nk2,1]=me/hbar*(np.transpose(np.conj(V[:,nu,nk1,nk2]))@Vpky(k[0],k[1])@V[:,mu,nk1,nk2]) #[fs/nm*eV]
