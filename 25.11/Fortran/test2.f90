...
subroutine EAlecntinteger(Elec, Alec)
    implicit none
    integer:: nt
    double precision, dimension(0:ntmax,0:1), intent(inout) :: Elec, Alec
    double precision :: time
    Alec(0,:)=0.
    ! Calculate Elec
    do nt=0,ntmax
    time = nt*deltat + tmin
	Elec(nt,0) = E0*exp(-(time)**2/TauL**2)*cos(omegaL*time)*cos(phi)
...
subroutine Elect(nt1, Et,Elec)
    implicit none
    real, intent(in) :: nt1
    double precision, dimension(0:1), intent(inout) :: Et
    double precision, dimension(0:ntmax,0:1):: Elec
...
subroutine Alect(nt1, At,Elec, Alec)
    implicit none
    real, intent(in) :: nt1
    double precision, dimension(0:1), intent(inout) :: At
	double precision, dimension(0:ntmax,2), intent(in) :: Elec, Alec
...
subroutine RHSsbeVG(RHS, rho,nt1,p,deltaE, Elec, Alec)
	use omp_lib
    	implicit none
	...
	!$OMP PARALLEL DO PRIVATE(nk2)
	do nk2 = 0, nkamax
		!$OMP PARALLEL DO PRIVATE(nk1,mu,nu)
		do nk1 = 0, nkamax
			do mu = 0, Norbital
				do nu = 0, Norbital
...
subroutine RK4(rho, nt, p, deltaE, Elec, Alec)
    implicit none
    ...
    double precision, dimension(0:ntmax,0:1) :: Elec, Alec
	allocate(RHS(0:Norbital,0:Norbital,0:nkamax,0:nkamax),k1(0:Norbital,0:Norbital,0:nkamax,0:nkamax)&
			,k2(0:Norbital,0:Norbital,0:nkamax,0:nkamax),k3(0:Norbital,0:Norbital,0:nkamax,0:nkamax)&
			,k4(0:Norbital,0:Norbital,0:nkamax,0:nkamax))
    ! Calculate k1
    
    call RHSsbeVG(RHS, rho, real(nt), p, deltaE, Elec, Alec)
    k1 = deltat * RHS

    ! Calculate k2
    call RHSsbeVG(RHS, rho + 0.5d0*k1, real(nt) + 0.5e0, p, deltaE, Elec, Alec)
    k2 = deltat * RHS

    ! Calculate k3
    call RHSsbeVG(RHS, rho + 0.5d0*k2, real(nt) + 0.5e0, p, deltaE, Elec, Alec)
    k3 = deltat * RHS

    ! Calculate k4
    call RHSsbeVG(RHS, rho + k3, real(nt) + 1.0e0, p, deltaE, Elec, Alec)
    k4 = deltat * RHS
...
subroutine calculatev(rho,nt,p,xi, Elec, Alec)
	use globalvar
	use para_MX2
	implicit none
	double complex, dimension(0:1):: I, Polar
	double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax), intent(in)  :: rho
	double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1), intent(in) :: p,xi
	...
	double precision, dimension(0:1) :: At
	integer, intent(in) :: nt
	allocate(velo(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1),N(0:Norbital))
	dS =(4.*pi/(sqrt(3.)*a*nkamax))**2*sqrt(3.)/2
	call Alect(real(nt),At,Elec, Alec)
	
	do j=0,1
		velo(:,:,:,:,j)=(p(:,:,:,:,j)-qe*At(j))/me
	end do
	...
	do nk2=0,nkamax
		do nk1=0,nkamax
			do nu=0,Norbital
				...
				N(nu)=rho(nu,nu,nk1,nk2)*dS
			end do
		end do
	end do
	end do
	write(1,'(E12.5, x,E12.5,x,E12.5,x,E12.5,x, E12.5)') tmin+nt*deltat, I(0), I(1)
	write(2,'(E12.5, x,E12.5, x,E12.5 ,x,E12.5 ,x,E12.5 ,x,E12.5 , x,E12.5)') tmin+nt*deltat, real(N(0)), real(N(1)),&
										 real(N(2)), real(N(3)), real(N(4)), real(N(5))
...
program main
	...
	do nt=1,ntmax
		...
		call RK4(rho, nt, p, deltaE, Elec, Alec)
		...
		if (mod(nt,int(10/deltat))==0) then
			...
			write(*,'(F14.2,F5.2,F5.2)') REAL(SUM(rho(0,0,:,:)+rho(1,1,:,:)+rho(2,2,:,:))),&
				sum(real(rho(1,1,:,:)/((rho(0,0,:,:)+rho(1,1,:,:)+rho(2,2,:,:))))),&
				sum(real(rho(2,2,:,:)/((rho(0,0,:,:)+rho(1,1,:,:)+rho(2,2,:,:)))))
		...
		call calculatev(rho,nt,p,xi, Elec, Alec)
		end if
	end do
	close(1)
	close(2)
