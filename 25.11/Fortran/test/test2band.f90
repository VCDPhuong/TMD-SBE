...
subroutine RHSsbeLG(RHSs, rhos , nt1s ,xis ,deltaEs, Elecs)
    use globalvar
use omp_lib
    implicit none
    ...
    double precision, dimension(0:ntmax) :: Elecs
    integer :: nu,mu,nk1
    real, intent(in) :: nt1s
    ...
    call Elect(nt1s, Ets ,Elecs)
    write(3,'(F14.5, x,F14.5)') tmin+nt1s*deltat, Ets
    !!$OMP PARALLEL DO
    !do nk2 = 0, nkamax
        !$OMP PARALLEL DO
        do nk1 = 0, nkamax
            do mu = 0, Norbital
                do nu = 0, Norbital
                    deltaEparts(nu, mu, nk1)= deltaEs(nu, mu, nk1)*rhos(nu, mu, nk1)
    ...
    if (mod(nt1s,(10/deltat))==0) then
    write(*,*) maxval(real(deltaEparts/hbar)), maxval(real(qe*lightinteractionparts/hbar)), maxval(real(gradrhos*qe/hbar*Ets))
    end if
...
subroutine RK4(rho, nt, xis, deltaEs, Elecs)
    use globalvar
    implicit none
    ...
    call RHSsbeLG(RHS, rho, real(nt), xis, deltaEs, Elecs)
    k1 = deltat * RHS

    ! Calculate k2
    call RHSsbeLG(RHS, rho + 0.5d0*k1, real(nt + 0.5e0),  xis, deltaEs, Elecs)
    k2 = deltat * RHS

    ! Calculate k3
    call RHSsbeLG(RHS, rho + 0.5d0*k2, real(nt + 0.5e0),  xis, deltaEs, Elecs)
    k3 = deltat * RHS

    ! Calculate k4
    call RHSsbeLG(RHS, rho + k3, real(nt + 1e0),  xis, deltaEs, Elecs)
    k4 = deltat * RHS
...
subroutine calculatev( rhos, nts, xis, Elecs)
    use globalvar
    implicit none
    double complex, dimension(0:Norbital,0:Norbital,0:nkamax), intent(in)  :: rhos
    double complex, dimension(0:Norbital,0:Norbital,0:nkamax), intent(in) :: xis
...
!    dS =(4.*pi/(sqrt(3.)*a*nkamax))**2*sqrt(3.)/2
    ...
    !            end do
                N(nu)=rhos(nu,nu,nk1)*dS/(2*pi)
            end do
            P=1/(2*pi)*(xis(0,1,nk1)*rhos(1,0,nk1)+xis(1,0,nk1)*rhos(0,1,nk1))
        end do
    !end do
    !end do
    !write(1,'(E12.5, x,E12.5,x,E12.5,x,E12.5,x, E12.5)') tmin+nt*deltat, I
    write(1,'(E12.5, x,E12.5, x,E12.5)') tmin+nts*deltat, P
    write(2,'(E12.5, x,E12.5, x,E12.5)') tmin+nts*deltat, real(N(0)), real(N(1))
...
program main
    ...
    do nt=1,ntmax
        ...
        !write(3,'(F14.5, x,F14.5, x,F14.5)') real(nt*deltat +tmin), Elec(nt)
        call system_clock(count_start, count_rate, count_max)
        start = count_start*1.0/count_rate
        call RK4(rho, nt, xi, deltaE, Elec)
        ...
        if (mod(nt,int(10/deltat))==0) then
            ...
            write(*,'(F14.2,F5.2)') REAL(SUM(rho(0,0,:)+rho(1,1,:))),&
        ...
        call calculatev( rho, nt, xi, Elec)
        !end if
    end do
    close(1)
    close(2)
