...
module varia
	...
	use globalvar
        implicit none
        !dummy,allocate
        double complex,allocatable,dimension(:,:) :: Vpkx, Vpky ,ham, ham_plus, ham_minus
        double complex,allocatable,dimension(:,:,:) :: Vpkxy
        double complex,allocatable,dimension(:,:) :: hamu,hamd
        double precision,allocatable,dimension(:) :: E_sorted
	double precision,allocatable,dimension(:,:) :: B
	integer,allocatable :: nk1, nk2, lwork, i, j, nu, mu,INFO 
	double precision,allocatable, dimension(:) :: array
	!
	!double precision,allocatable,dimension(:,:,:),intent(inout) :: E
	!double precision,allocatable,dimension(:,:,:,:),intent(inout) :: deltaE
	!double complex,allocatable,dimension(:,:,:,:,:),intent(inout) :: p,Xi
	!double precision,allocatable,dimension(:,:,:),intent(inout) :: grid
	!integer,allocatable, dimension(:,:),intent(inout) :: Delta
	!double complex,allocatable,dimension(:,:,:,:),intent(inout) :: V
	!keep variable, not allocate
	...
	do nk2 = 0,nkamax
		...
		close(1)
		deallocate(Vpkx, Vpky ,ham, ham_plus, ham_minus,Vpkxy, hamu,hamd,E_sorted,B, nk1, nk2, lwork, i, j, nu, mu,INFO,array)
    end subroutine
end module
module defud
use globalvar
...
subroutine RK4(rho, nt, p, deltaE)
    ...
    double precision, dimension(:,:,:,:), intent(in) :: deltaE
...
module calculatevariable
	...
	double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax), intent(in)  :: rho
	double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1), intent(in) :: p
	double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1):: v
	...
	do nu=0,Norbital
		do mu=0,Norbital
			do nk1=0,nkamax
				do nk2=0,nkamax
					I(j)=I(j)+qe*v(nu,mu,nk1,nk2,j)*rho(mu,nu,nk1,nk2)*dS
				end do
			end do
		end do
	end do
