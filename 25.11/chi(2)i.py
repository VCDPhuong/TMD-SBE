    ...
    def Chi2(omega):
        ...
        for m in range(Ndimension):
            for n in range(Ndimension):
                dummy2[...,m,n]+=np.einsum('ij...,jk...->ik...',Xi[...,m],p[...,n])-np.einsum('ij...,jk...->ik...',p[...,n],Xi[...,m])
        for i in range(Ndimension):
            for j in range(Ndimension):
                for k in range(Ndimension):
                    for ci in conduct:
                        for vi in valence:
                            Chi2[i,j,k]+=(1j*qe**3/(hbar*me)**2)*np.sum(np.multiply(p[ci,vi,:,:,k]/(deltaE[ci,vi,:,:]/hbar-omega-1j*gamma),dummy2[vi,ci,:,:,i,j]))*dS*1.6
        return Chi2
    for j in range(int(4/deltaomega)):
...
fig.savefig('Chi2.svg', format='svg', dpi=1200)
fig.savefig('Chi2.eps', format='eps')
#plt.show()