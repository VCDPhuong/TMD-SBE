...
module globalvar
    ...
    module para_MX2
     ...
     module varia
        ...
        integer :: nk1, nk2, i, j, nu, mu,INFO 
        ...
        do nk2 = 0,nkamax
            ...
            do nk2 = 0,nkamax
                do nk1 =0,nkamax
                write(1,'(I4, 2x, I4, 2x, E12.5, 2x, E12.5)') nk1, nk2, grids(nk1,nk2,0), grids(nk1,nk2,1)
                end do
            end do
            close(1)
            print *, "grid done"
                    ...
                    do nu =0, Norbital
                        do mu= 0,Norbital
                            if (Delta(nu,mu)/=0) then
                               ...
                               xis(nu,mu,nk1,nk2,0)=0.
                               xis(nu,mu,nk1,nk2,1)=0.
                            end if
                        end do
                    end do
                end do 
            end do
            !print*, "p calculate: done"
            do nu = 0, Norbital
                do mu = 0, Norbital
            ...
            open(unit=1,FILE='band1.txt')
                do i=1,nkamax
                    do j=1,nkamax
                        write(1,'(8E12.5)')grids(i,j,0),grids(i,j,1),&
                                        Es(0,i,j),Es(1,i,j),Es(2,i,j),&
                                        Es(3,i,j),Es(4,i,j),Es(5,i,j)
                    end do
                end do
    ...
    subroutine commu(A, B, C)
        ...
        do m = lbound(A,3), ubound(A,3)
            do n = lbound(A,3), ubound(A,3)
                ...
                C(:,:,n,m) = matmul(A(:,:,n,m), B(:,:,n,m)) - matmul(B(:,:,n,m), A(:,:,n,m))
            end do
        end do
            !$OMP END PARALLEL DO
    ...
    !end subroutine
        subroutine RHSsbeVG(RHSs, rhos , nt1s ,ps ,deltaEs, Ats)
            ...
            write(3,'(F14.5, x,F14.5)') nt1s, Ats
            !$OMP PARALLEL DO
            do nk2=0,nkamax
                do nk1 = 0, nkamax
                    do mu = 0, Norbital
                        do nu = 0, Norbital
                            deltaEparts(nu, mu, nk1, nk2)= deltaEs( mu, nu, nk1, nk2)*rhos(nu, mu, nk1, nk2)
                        ...
                        do nk1=0, nkamax
                            do mu =  0,Norbital
                                do nu = 0, Norbital
                                    ...
                                    P(j)=P(j)+1/(2*pi)**2*(xis(nu,mu,nk1,nk2,j)*rhos(mu,nu,nk1,nk2))
                                    I(j)=I(j)+1/(2*pi)**2*(velo(nu,mu,nk1,nk2,j)*rhos(nu,mu,nk1,nk2))
                                end do
                            end do
                        end do
                    end do
    ...
    program main
        ...
        !        write(*,'(F14.2,F3.2,F14.2,F3.2,F14.2,F3.2,F14.2,F3.2,F14.2,F3.2,F14.2,F3.2)') &
        !                                            SUM(rho(0,0,:,:)),SUM(rho(1,1,:,:)),SUM(rho(2,2,:,:)),&
        !                                            SUM(rho(3,3,:,:)),SUM(rho(4,4,:,:)),SUM(rho(5,5,:,:))
        !    end if
