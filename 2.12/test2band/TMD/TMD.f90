...
module globalvar
    ...
    module para_MX2
     ...
     module varia
        ...
        use globalvar
        implicit none
        double precision,dimension(0:Norbital,0:nkamax,0:nkamax),intent(inout) :: Es
        double precision,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax),intent(inout) :: deltaEs
        double complex,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1),intent(inout) :: ps, xis
        double precision,dimension(0:nkamax,0:nkamax,0:1),intent(inout) :: grids
        integer,dimension(0:Norbital,0:Norbital),intent(inout) :: Deltas
        double precision,dimension(0:1,0:1) :: B
        double complex,dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax),intent(inout) :: Vs
        double complex, dimension(0:Norbital,0:Norbital) :: Vpkx, Vpky
        double complex, dimension(0:Norbital,0:Norbital,0:1) :: Vpkxy
        double complex, dimension(0:halfNorbital,0:halfNorbital) :: hamu,hamd
        ...
        do nk2 = 0,nkamax
            ...
            do nk2 = 0,nkamax
               do nk1 =0,nkamax
                    ...
                    do mu = 0, Norbital
                        do nu = 0, Norbital
                        !print*,  maxval(real(Vpkx(:,:))), me / hbar
    ...
    !end subroutine
                    ...
                    do mu = 0, Norbital
                        do nu = 0, mu
                            if (nu == mu) then
                                ...
                                deltaEparts(nu, nu, nk1, nk2) = deltaEs(nu, nu, nk1, nk2) * rhos(nu, nu, nk1, nk2)
                            else
                                ...
                                deltaEparts(mu, nu, nk1, nk2) = CONJG(deltaEparts(nu, mu, nk1, nk2))
        ...
        double precision, dimension(:,:), intent(in) :: Ats
        real :: time
        allocate(RHS(0:Norbital,0:Norbital,0:nkamax,0:nkamax),Rdummy(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:4))
        ! Calculate k1
    ...
    module calva
        ...
        subroutine calculatev( rhos, nts, xis, ps, Ats)
            ...
            implicit none
            double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax), intent(in)  :: rhos
            double complex, dimension(0:Norbital,0:Norbital,0:nkamax,0:nkamax,0:1), intent(in) :: xis, ps
    ...
    program main
        ...
        Real :: start, finish
        integer :: nt
        allocate(rho(0:Norbital,0:Norbital,0:nkamax,0:nkamax))
        rho(0,0,:,:)=complex(1.,0.)
        rho(1,1,:,:)=complex(0.,0.)
        rho(2,2,:,:)=complex(0.,0.)
        rho(3,3,:,:)=complex(1.,0.)
